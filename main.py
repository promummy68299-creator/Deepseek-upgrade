...
def run_http_server():
    ...
    try:
        ...
        try:
            ...
            logger.info(f"HTTP server started on port 8080 as fallback")
            server.serve_forever()
        except Exception as e2:
            logger.error(f"Failed to start HTTP server on fallback port: {e2}")
...
def main():
    ...
    try:
        ...
        http_thread.start()
        logger.info(f"HTTP server thread started on port {Config.PORT}")
